...
\begin{definition}[Mollifier-Kernel]
...
\draw[black, domain=-1:1, samples=500] plot (\x, {3*exp(-1/(1-abs(\x)*abs(\x)+0.001))}); 
% \draw[black, domain=-1:1, samples=500] plot (\x, {1.5*exp(-1/(1-abs(\x*1.2)*abs(\x*1.2)+0.001))}); 
 \end{tikzpicture}    
   \end{center}
...
\begin{definition}[Mollification Operator]
...
\begin{proof}
 ...
 .\end{align*}
\end{proof}
An example of how Mollification can be useful consider the following example : 
\begin{example}
  Consider the following step function $\cha_{[-1,1]}(*)$ , then the mollification $J_{\epsilon }(u)$ will look like this
\begin{figure}[H]
   \begin{center}
 \begin{tikzpicture}
\begin{axis}[
...
\end{example}
\begin{exercise}
 Proof that the Mollification of the step function $\cha_{[-1,1]}(*)$  is a smooth function
\end{exercise}
\hspace{0mm}\\
